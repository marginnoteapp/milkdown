...
const createEmptyStyle = (themeManager: ThemeManager, { css }: Emotion) => {
  const palette = (color: Color, opacity = 1) =>
    themeManager.get(ThemeColor, [color, opacity])
...
export const createProps = (status: Status, utils: ThemeUtils) => {
  return {
    ...
    decorations: (state: EditorState) => {
      const paragraph = findParentNode(({ type }) => type.name === "paragraph")(
        state.selection
      )
      ...
      if (
        decorations != null &&
        decorations.find(state.selection.from, state.selection.to).length > 0
      ) {
        status.clear()
        return null
      }
      ...
      if (
        ...
        state.selection.$from.parentOffset !==
          paragraph.node.textContent.length ||
      ...
      const emptyStyle = utils.getStyle(emotion =>
        createEmptyStyle(utils.themeManager, emotion)
      )
      const slashStyle = utils.getStyle(emotion =>
        createSlashStyle(utils.themeManager, emotion)
      )

      if (actions.length) {
        return createDecoration(placeholder, [
          emptyStyle,
          slashStyle,
          "empty-node",
          "is-slash"
